...
public static class RiderProducerExtensions
{
    ...
    public static void AddProducers(this IRiderRegistrationConfigurator riderConfiguration, Assembly[] assemblies)
    {
        ...
        {
            GetConfiguration(assemblies, kafkaProducer, out var producerConfig, out var @delegate);
            ProducerConfigure(riderConfiguration, kafkaProducer, producerConfig, @delegate);
        }
    }
    /// <summary>
    ...
    private static void ProducerConfigure(this IRiderRegistrationConfigurator riderConfiguration, Type kafkaProducer,
    ...
    {
        return assemblies
            .SelectMany(listOfAssemblies => listOfAssemblies.GetTypes()).FirstOrDefault(t =>
                t.GetInterfaces().Any(type => type.IsGenericType &&
                                              type.GetGenericTypeDefinition() == (typeof(IKafkaProducerConfiguration<>)) && 
                                              type.GetGenericArguments()[0].FullName.Contains(assembly.FullName)));
    }
